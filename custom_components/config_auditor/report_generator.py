...
def _load_translations_from_json(language: str) -> dict:
    ...
    if not path.exists():
        path = base / "en.json"
    try:
        return _json.loads(path.read_text(encoding="utf-8")).get("report", {})
    except Exception:
        return {}
...
"""
        ...
        report += f"\n\n---\n\n*{self._t('report_generated_by')} v1.1.0 - {timestamp_display}*\n"
    ...
    async def _generate_pdf_with_timestamp(
        ...
        *,
        ...
        pdf.cell(0, 8, f"{t('report_generated_by')} v1.1.0 - {timestamp_display}", ln=1)
