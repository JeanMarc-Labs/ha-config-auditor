...
async def _tool_ha_create_blueprint(hass: HomeAssistant, params: dict) -> dict:
    ...
    try:
        ...
        # The name is derived from the blueprint's: a second run overwrites.
        backup = await _async_write_file(hass, bp_path, _bp_header + bp_yaml)
    except Exception as exc:
...
async def _tool_ha_update_blueprint(hass: HomeAssistant, params: dict) -> dict:
    ...
    try:
        backup = await _async_write_file(hass, str(fpath), new_yaml)  # verbatim
    except Exception as exc:
...
async def _tool_ha_remove_blueprint(hass: HomeAssistant, params: dict) -> dict:
    ...
    try:
        backup = await _async_remove_file(hass, fpath)
    except Exception as exc:
    ...
    return {
        ...
        "deleted": fpath,
        "backup": backup,
        "message": f"Blueprint deleted: {fpath}. A copy was kept at {backup}.",
    }
...
async def _tool_ha_import_blueprint(hass: HomeAssistant, params: dict) -> dict:
    ...
    try:
        ...
        # Re-importing a blueprint of the same name overwrites the file.
        backup = await _async_write_file(hass, out_path, _import_content)
    except Exception as exc:
        return {"error": f"Failed to write blueprint: {exc}"}
