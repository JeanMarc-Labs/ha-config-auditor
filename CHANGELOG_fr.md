...
- **L'analyse des automatisations indique désormais quelle étape a pris le temps** — la ligne de scan ci-dessus chronométrait cette analyse d'un seul bloc ; elle rend maintenant compte de ses propres étapes, de la plus lente à la plus rapide (doublons, blueprints, jamais déclenchées, et les autres), en INFO quand l'analyse a dépassé deux secondes, en DEBUG en dessous.
