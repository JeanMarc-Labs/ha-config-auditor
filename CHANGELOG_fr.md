...
- **Le relevé des références d'entités gelait Home Assistant dix secondes d'affilée** — l'étape qui lit chaque automatisation et chaque script pour savoir quelles entités ils utilisent s'exécutait entièrement sur la boucle d'événements : 9,9 s d'une analyse d'entités de 10,0 s sur un Raspberry Pi 3, en un seul gel pendant lequel Home Assistant ne répondait plus à rien. Elle tourne maintenant dans un thread, et la seconde passe qui relisait chaque configuration sous forme de texte — le filet qui attrape les identifiants d'entités cachés sous des clés inattendues — est fondue dans le même parcours : la configuration est parcourue une fois au lieu de deux, 38 % de travail en moins pour cette étape. **Le scan ne raccourcit pas beaucoup ; il cesse de bloquer.**
