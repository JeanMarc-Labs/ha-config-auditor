...
- **The automation analysis now says which of its steps took the time** — the scan line above timed that analysis as one block; it now reports its own steps, slowest first (duplicates, blueprints, never-triggered, and the rest), on INFO when the analysis ran for more than two seconds and on DEBUG below that.
