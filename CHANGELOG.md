...
- **A value written by an AI agent could come back as something else, and disable the automation** — the MCP tools and the panel's fixes wrote `on`, `off`, `yes`, `no` or a time like `22:00:00` without quotes, and Home Assistant reads those as a boolean or a number. A state trigger `to: off` was then rejected on reload and the automation disabled; `hvac_mode: off` silently did nothing. Such values are now written in quotes, and the bare ones already in your files are left as they are. An automation written before this fix keeps its bare value: if Home Assistant reports it as invalid, put the value in quotes.
- **An automation written by an AI agent could be disabled on reload while the tool reported success** — Home Assistant's reload succeeds even when it disables an invalid automation, so H.A.C.A's rollback never ran. The MCP tools now check each automation, script and scene with the validation of Home Assistant's own editor before writing it: one Home Assistant would reject is not written, and the agent gets Home Assistant's error.
- **A fix applied from the panel could disable the automation it fixed, and report success** — removing a zombie entity could leave a state trigger with no entity, and the AI optimizer wrote whatever the AI returned, a bare `to: off` included; Home Assistant disables such an automation at the next reload, without an error. Every panel fix, the alias and description fix and the optimizer now go through the same check as the MCP tools: what Home Assistant would refuse is not written, and the panel shows its reason. A failed reload after a zombie-entity or alias fix also puts the file back.
- **Converting a device trigger, condition or action to its entity could change what the automation did** — only a dozen types were known: a *door open* condition lost the state it checks, a ZHA button press became a trigger on the device's first entity, a light's *brightness up* called a service that does not exist, and a trigger's `id` was dropped. The conversion now does what Home Assistant itself does with each device block, for every entity domain; one with no equivalent, such as a ZHA button, stays as it is and the preview says why.
- **A device block inside a `choose:`, `if:`, `repeat:` or `parallel:` was neither reported nor converted** — only the top level of an automation was looked at, so a device condition in a `choose:` option had no issue and no fix. Such blocks are now reported where they are, and the device_id fix converts them in place.
- **The confirmation shown after applying an AI-suggested alias or description was always in French** — it is now in your language.
- **An AI agent could not apply any fix** — `haca_apply_fix` and `haca_fix_batch` called the fix services with a field their schema refuses, so every device_id, mode and template fix failed, and a fix that wrote nothing would have been reported as applied. The tools now fix the block the issue names and report Home Assistant's reason when nothing is written; issue IDs are the panel's, so two issues on one automation no longer share one.
- **The fix preview showed Python objects instead of YAML** — since 1.8.0, the *before* and *after* panes of the device_id, mode and template fixes, and the YAML sent to the AI for a description, read `!!python/object/apply:ruamel…`. They show the automation as it will be written.
- **Editing a YAML file rewrote it from top to bottom** — every list was re-indented and every long value re-wrapped to one fixed style, so a one-line change showed up as the whole file in a diff. A file is now written back in its own layout, whether Home Assistant's editor or you wrote it: only the lines that changed differ.
- **The backup taken before an AI agent's changes failed on every Core install, and was reported as started** — each MCP write tool first started a full Home Assistant backup asking for every add-on and folder, which a Core install refuses; the refusal only reached the log. The tools no longer start a full backup: each one copies the file it changes into `.haca_backups` first and returns the copy's path, and the configuration-file and blueprint tools, which had no copy at all, now take one too. `ha_backup_create` remains for a backup you ask for: it now works on Core (configuration and database) and reports Home Assistant's refusal instead of "started".
- **The Backups tab could restore a script file's backup over an automation file** — a backup was named after its file's name alone, so on a split config `scripts/lights.yaml` and `automations/lights.yaml` shared one name in the list and one quota of 10. Each file's backups now sit in a folder at that file's path under `.haca_backups/files/`, which is where a restore writes: the tab lists and restores script, scene, blueprint and configuration files too. Backups taken before 1.8.1 are still listed and restored as before.
- **A restore from the Backups tab that failed was announced as done** — the panel never read the answer of the restore. It now shows the error.
- **More than nine backups of one file in the same second deleted the newest ones** — pruning sorted `_10` before `_2`, and a new backup could take the name of one just pruned and be deleted as it was written; a failed reload then had nothing to roll back to. A batch of fixes on one file could hit it.
- **The *Auto-backup before fix* switch in Configuration did nothing** — no code ever read it: every fix takes a backup regardless. The switch is gone.
...
- **Continuous integration** — every push and pull request runs the HACS check, the test suite and `ruff`. Nothing checked this repository before, which is how the 1.7.7 split-config fix reached a release having missed two functions. `requirements_test.txt` makes the suite runnable locally in one command.
- **Every scan now logs how long it took, and how much of that time Home Assistant was frozen** — nothing in H.A.C.A measured the scan, so there was no way to tell whether it is worth moving off Home Assistant's event loop. Two INFO lines per scan: the stages, slowest first, and how long the event loop was actually blocked, with the stage responsible for the worst freeze. The two are different questions — a stage takes wall-clock time both while it holds the loop and while it waits its turn on a busy one, and only the first is a freeze. The per-analyzer breakdown of the parallel phase is there too, under `debug_mode`.
- **The two long analyses now say which of their steps took the time** — the scan line above timed each of them as one block. The automation and entity analyses now report their own steps, slowest first (duplicates, blueprints, references, states, and the rest), on INFO when the analysis ran for more than two seconds and on DEBUG below that. Both entries below came out of reading those lines.
...
- **Any analysis type you re-enabled came back off after a restart** — `async_setup_entry` merged the 14 default exclusions back into `excluded_issue_types` on every start, so the panel saved your choice and the next Home Assistant start overwrote it. The defaults are now seeded once, by a config-entry migration; the list belongs to you afterwards.
- **A blueprint automation with an empty required input killed the whole automation analysis** — the check built its message with `t("blueprint_empty_input", key=input_key)`, and `key` was also the name of the translator's own first parameter, so the call raised `TypeError` before it could produce the finding. Nothing between there and the coordinator catches that: the scan logged `analyze_all() CRASHED` and the panel went on showing the previous scan's automation findings. The translator's `key` is positional-only now, so a translation carrying a `{key}` placeholder behaves like any other.
- **Entities used inside a `choose:` branch were reported as unused** — the reference scan read only the first level of `trigger` / `condition` / `action`, so anything nested in `choose:`, `if / then / else`, `repeat:`, `parallel:`, a nested `sequence:`, a `default:` or a service `data:` block was invisible, and Jinja templates and `target:` device / area / label selectors were never read at all. On a real installation that means perfectly used entities flagged as zombie, orphaned or unused. The scan is now fully recursive and resolves device, area and label targets through the registries. **Expect the entity issue count to drop after the next scan.** Zombie detection still relies on explicit `entity_id:` fields only, so a template match can never invent a missing entity.
- **A category with more than 200 issues showed 200 of them and said nothing** — the server capped each list at 200 while the tile above it counted the real total, so "247 problems" sat on top of a list of 200 with nothing to explain the gap. The cap is now 2000, and the panel says "*X* of *Y* shown" whenever a list is still short of its counter.
- **The Helpers tab was always empty** — the audit computed `helper_issue_list` and the panel rendered it, but the websocket answer never carried it; the Helpers tile counted 0 while those issues were still part of the total. It is now part of the response like every other category.
- **Asking `haca/get_data` for one category returned every *other* category in full** — the condition was inverted, so narrowing the request made the answer bigger. The correct helper had been sitting one function above it, never called.
- **"Fix with AI" did nothing useful on a split config** — both handlers still opened `<config>/automations.yaml` or `scripts.yaml`, so the suggestion was generated with no YAML context and applying it raised `No such file or directory`. They now locate the file that really holds the entry, and a miss reports how many files were searched.
- **Applying an AI field fix erased every comment in the file** — the whole file was read with `safe_load` and written back with `dump`, which drops comments, key order and quoting style, and no backup was taken first. The edit is now a ruamel round-trip on the one file that holds the entry, a backup is taken before writing, and a file carrying `!secret` or `!include` is skipped instead of being rewritten.
- **Editing an automation, script or scene from the panel or an AI agent erased every comment in its file** — the refactoring services, the automation optimizer and the nine MCP write tools each read the whole file with `safe_load` and wrote it back with `dump`, which drops comments, blank lines, key order and quoting style across the file, not only around the entry being changed. All of them now edit through a ruamel round-trip and write atomically, and a file carrying `!secret` or `!include` is skipped rather than rewritten.
- **Updating or deleting a script or a scene reported success when the reload had failed** — the reload error was swallowed, so Home Assistant kept running the old definition while the tool said the change was applied. Both now restore the file and report the failure, as the automation tools already did.
- **A backup taken by the automation optimizer could not be restored** — it was named `<file>_optim_<timestamp>.yaml`, which the restore path could not resolve back to a source file: the panel offered it and then refused to put it back. Optimizer snapshots now carry the same name as every other one.
- **"365 days of history" kept about 15 days** — `history_retention_days` was compared against the number of stored snapshots, and the default 60-minute scan interval produces 24 of those per day. Snapshots are now dropped on their own timestamp, with a hard ceiling of 5000 entries to keep `.storage` bounded.
...
- **Eleven server-side messages were written in French in the code** — the MCP explain-issue prompt, the summaries of the report-deleted and backup-deleted notifications, the mode-change preview and the automation optimizer's five YAML errors reached every user in French whatever their language. All eleven are translated in the 13 languages. The MCP tool descriptions, which only the AI model reads, are normalised to English like the sixty others.
- **The MCP tab listed 67 tools where the server registers 69** — the panel carried its own copy of the tool list, and a "67 tools" label spelled out in all 13 languages; neither was tied to the server, and two of the nine `ha_*` aliases had been missing from the list since they were added. Both figures now come from the server, which reports what an agent actually discovers (60 advertised tools) alongside what it can call (69, aliases included), and a test fails the build if the panel's grouping drifts again.
- **The Home Assistant log filled up with HACA traces logged as warnings** — the dashboard analysis alone wrote about twenty WARNING lines per scan (`analyze_all() END — 0 total issue(s)` among them), and the AI, chat, scan-lock and refactoring paths logged every expected fallback the same way, so Settings → System → Logs showed H.A.C.A as an integration permanently in error. The fifty-one lines that only traced normal behaviour are now DEBUG, WARNING is kept for what actually degrades an audit, and the health-score regression — already delivered as a notification — moved to INFO.

### Changed

- **Probable duplicates are reported once per automation instead of once per pair** — similar automations come in families where every member matches every other, so ten "motion turns this light on" automations, one per room, produced ninety findings that said the same thing nine times over; a hundred of them produced 9 900. Each automation now carries a single finding naming the others it resembles and how close the nearest one is — the shape the exact-duplicate finding already used. **Expect the automation issue count to drop sharply if you have families of similar automations.**
...
- **Two pieces of dead code removed** — a naming-convention check that only accepted Latin letters, so « Гостиная - Лампа » failed a convention it actually follows, and a 95-line automation-description builder. Neither had been called since the first commit, and both carried the last hardcoded French in the package.
- **The Repairs module is tested again** — its 30 tests targeted a fix-flow design dropped before 1.7 and had been skipped ever since, while the module itself ran on every scan of every installation. They are rewritten against what it does now — 22 tests, each checked against a deliberately broken copy of the module — and two older tests that called it inside a `try` and asserted nothing are gone. The suite is 694 tests with none skipped.
- **One way to write a YAML file** — every path that edits a configuration file now goes through a single module: round-trip read, atomic write, a snapshot beforehand, and a documented refusal to touch a file whose tags it cannot safely rewrite. Snapshots are also pruned per source file rather than ten in total, so on a split configuration a busy file no longer evicts another one's restore points.
- **The MCP server is a package, not a 6 000-line file** — sixty tools, their JSON schemas, the HTTP views and the helpers they share all lived in `mcp_server.py`, 263 KB and by far the hardest file in the project to review. It is now twelve modules under `mcp_server/`, one per tool family, with the tool list and the handler map assembled in a single place; every definition was moved unchanged, and what the server advertises and dispatches is byte-for-byte what it was.
- **90 lines of dead code removed from the MCP server** — a bearer-token check superseded when the views moved to Home Assistant's own auth middleware, and two YAML helpers left unused by the write unification above. The rollback guard that pinned one of them now covers the helper that replaced it.
- **The HACS check now verifies the repository topics** — it was told to ignore them because the repository had none. It has them now, so the check is real. The `brands` requirement stays ignored: it belongs to the default store, which H.A.C.A is not in.
- **The README now documents what H.A.C.A exposes** — installing it opens an MCP endpoint, offers 60 tools to any conversation agent and can send a periodic report to an AI, and none of that was written down anywhere. A **Security & privacy** section now lists every surface with who can reach it, what leaves the instance for an LLM and when — including the fallback chain that retries the next provider you have configured — and how to switch each one off. Three stale facts went with it: the advertised MCP URL was `/api/haca/mcp` (it is `/api/haca_mcp`), the tool count read 65 (the server serves 60), and the link to the French README was dead.

### Performance

- **The blueprint check re-read and re-parsed the same blueprint for every automation using it** — a blueprint exists to be reused, so a house running 80 automations on 5 blueprints parsed 80 files where 5 would do, and parsed them on the event loop: measured at 5.9 s of a 6.3 s automation analysis on a Raspberry Pi 3. Each blueprint is now read once per scan, in the executor. 5x to 17x faster, and the cost follows the number of blueprints rather than the number of automations.
- **Every scan re-read every blueprint** — reading each file once per scan, above, took care of the repetition inside one scan but not between them: with many distinct blueprints, each scan still read and parsed all of them, 5.8 s of the automation analysis on a Raspberry Pi 3. The parse is kept from one scan to the next now, and refreshed only when the file's date or size changes, so an unchanged blueprint costs a single `stat()`.
- **Ten seconds of every scan went into a deprecation warning that is never printed** — the map of which entities belong to which device looked each device up through `device_registry.devices`, a mapping Home Assistant has deprecated. Every call on it walks the entire Python stack to work out which integration made it, and the warning that comes out is logged only once — so the log stayed quiet while the walk kept happening, once per entity. On a Raspberry Pi 3 with 539 entities that was **10.1 s of event loop per scan**, on a configuration of twelve automations. The registry is read the supported way now, in all three places HACA touches it. **This was the freeze**: on that instance a scan now blocks the event loop for 0.7 s instead of 19.7 s, and the entity analysis it was billed to fell from 10.4 s to under a tenth of a second.
- **The entity reference walk ran on the event loop** — the step that reads every automation and script to work out which entities they use held the loop from end to end, whatever the size of the configuration. It runs in a worker thread now, and the second pass that re-read each configuration as text — the net that catches entity ids hiding under unexpected keys — is folded into the same walk, so the configuration is read once instead of twice: 38% less work for that step.
- **Duplicate detection compared every automation to every other one** — the near-duplicate check ran a Jaccard similarity over all n(n−1)/2 pairs, in a synchronous method that cannot hand the event loop back while it runs, and the new scan instrumentation measured that tail of the automation analysis inside a 12.6 s freeze. Identical automations are now compared once instead of pair by pair, and each one only meets those sharing one of its rarest structural tokens: the same pairs are found, 4 to 11 times faster.
- **The panel called the backend every 60 seconds for data that changes every 60 minutes** — each call returned the whole dependency graph, the full battery list, every complexity score and the scene and blueprint stats, unpaginated; 59 answers out of 60 were identical to the one before. The coordinator now announces the end of every scan, scheduled ones included, and the panel reloads on that event; the timer is only a 5-minute safety net, and an answer describing a scan already on screen is not redrawn.
- **The battery library was scanned end to end for every device** — 2140 entries, with `strip().lower()` recomputed on each of them, for each battery device: about 640 000 iterations per scan on an installation with 300 of them. The entries are normalised once at load time and indexed by manufacturer, so a lookup only walks the handful of entries sharing the device's brand — same results, around 200 times faster.
- **`haca/get_translations` re-read a 130 KB file from disk on every panel open** — while all 13 languages had already been parsed into memory at startup. It is served from that cache now.
- **The unused-helper check scanned the whole config as text, per helper** — with 100 helpers and a 5 MB config that is 500 MB of substring searching per audit. The entity ids are extracted once into a set; the check is now a hash lookup.
- **The `haca_ignore` label set was rebuilt by each of the seven analyzers** — seven full walks of the entity and device registries per scan. It is now computed once per scan and shared.
- **Blocking filesystem calls in the event loop** — the blueprint directory walk, the path guards of the MCP file tools, the panel bundle probe, the backup path checks and the report directory creation all ran on the event loop, where Home Assistant logs a warning for each. They now run in the executor, grouped so a check that used to cost four calls costs one.
- **Panel language changes wrote to `.storage/core.config_entries`** — the language used for background notifications follows the panel, so two admins in two languages opening the panel in turn produced one disk write each. Persisting it is now capped at one write per hour, and the language of a websocket answer follows the user who asked rather than the last person to open the panel.
...
- **Blueprint tools: arbitrary file read, write and delete through MCP** — `ha_get_blueprint` and `ha_update_blueprint` accepted any absolute path with no check, and `ha_remove_blueprint`'s guard compared the *joined* path with `startswith()`, so `../../secrets.yaml` — whose joined form still begins with `/config/blueprints/` — passed it, and `os.remove` then resolved the `..`. Any file in the config directory could be read into an MCP response, overwritten, or deleted by whatever drives these tools. All three now resolve the path (symlinks and `..` included) and require it to sit inside `/config/blueprints/`. Blueprint directories are also created `0o755` instead of `0o777`.
- **Scripts and scenes were audited as if there were none** — with `script: !include_dir_merge_named ha_scripts/` (or any other `!include_dir_*` form) the loaders looked at `<config>/scripts.yaml`, found nothing and returned: fifty real scripts reported as zero issues, silently, which reads as a clean bill of health. Automations were half-handled — one regex recognised `!include_dir_merge_list` only, and a stale `automations.yaml` that HA no longer reads was audited anyway, producing findings on automations that do not exist. All four directory forms, `!include` to a single file, and labelled sections (`automation ui:` + `automation manual:`) are now resolved for the three domains through one shared resolver. **If you run a split config, re-run a scan: the results will change.**
- **MCP write tools edited a file Home Assistant no longer reads, then reported success** — `ha_create_automation` appended to `<config>/automations.yaml` whatever the layout, called `automation.reload`, and returned `{"success": true, "entity_id": …}` for an automation that never appeared; `ha_update_automation` and `ha_remove_automation` did the same, and the script and scene tools failed outright with "scripts.yaml not found" for scripts that exist and run. Each now locates the file that actually holds the entry and writes back only there; a new entry goes to the flat file on a classic install, or to a dedicated `haca_mcp.yaml` inside the merged folder on a split one. A miss returns an error naming how many files were scanned, instead of a false success. `ha_deep_search` and the orphan-helper check read every file too — on a split config the latter flagged every helper as unused.
- **The compliance checks and the AI complexity analysis read the flat files too** — "automation without a description", "automation without an `id`" and "script without a description" all opened `<config>/automations.yaml` / `scripts.yaml` directly, so they reported nothing at all on a split config; the complexity analysis showed "YAML unavailable" for the same reason. Both now go through the shared resolver. The complexity analysis also never found a script even on a flat install: it parsed `scripts.yaml`, a named mapping, as if it were a list.
- **`ha_update_script`, `ha_remove_script`, `ha_update_scene` and `ha_remove_scene` crashed before doing anything** — each began with a bare `scripts_path` / `scenes_path` statement referencing a local assigned two lines later, raising `UnboundLocalError` on every call. The four tools were dead on arrival; the stray lines are gone.
- **Blueprint tools failed on every blueprint that uses `!input`** — `!input` is a standard Home Assistant tag with no PyYAML constructor, so `safe_load` raised on it: `ha_get_blueprint` returned an error, `ha_list_blueprints` returned `{"path", "error"}` per file with no name or inputs, and `ha_import_blueprint` refused most community blueprints. They now parse with Home Assistant's own loader and write the YAML text verbatim, so `!input` survives a round-trip. **`ha_update_blueprint` now requires the full `yaml=` text**: its field-level parameters (`name`, `inputs`, `triggers`, …) went through a `yaml.dump()` that cannot represent `!input`, and have been removed from the tool schema rather than left to mangle files silently.
...
- **The auto-generated dashboard opened on "Custom element not found: haca-dashboard-card"** — HACA waited for `lovelace.resources.loaded` before registering its card, but that flag is only raised inside Home Assistant's own `lovelace/resources` handler — that is, once a browser has already fetched and imported the resource list for the page — so the registration always landed one page load too late. The collection is now loaded explicitly at setup, and the panel imports the card module into the parent document before saving the dashboard, so the card renders in the very session that created it, without a reload. If the module still cannot be loaded, the card is left out rather than producing an error card.
- **`ha_reload_core` never reloaded the core configuration** — `domain="core"` called `core.reload_config_entry`, an action that has never existed under that domain, and `domain="customize"` called `customize.reload`, which does not exist either; both are sections of `configuration.yaml`, not integrations, and both now go through `homeassistant.reload_core_config`. Every other domain is checked against the service registry first, so an instance with no `template:` section gets an explanation instead of a raw "Action template.reload not found".
- **`ha_call_service` could not call the actions that return data** — the call never passed `return_response`, so every response-only action (`weather.get_forecasts`, `calendar.get_events`, `todo.get_items`, `conversation.process`, …) came back as "requires responses and must be called with return_response=True", and the ones where the response is optional ran but had their answer thrown away. The tool now reads the action's declared response support and returns the payload in a `response` field.
- **A `.yml` file in a merged folder produced findings for entries that do not exist** — Home Assistant's four `!include_dir_*` constructors glob `*.yaml` only, but the resolver also matched `.yml`, so a `.yml` dropped into a split folder was audited as live configuration although HA never loads it. It is no longer scanned, and it is now reported as an issue of its own ("YAML file never loaded") instead of being dropped in silence: a file Home Assistant skips is exactly what an audit should be surfacing.
- **A single `!secret` line made a whole file invisible to the audit** — the inspection loader is Home Assistant's own, which refuses `!secret` outside a secrets cache, and every caller caught the exception and moved on. One `!secret` in a scripts file therefore hid every script in it — no issues, no message, the same silent all-clear that split configs used to produce. Such a file is now re-read with a loader that turns each tag into a placeholder (`!secret db_password` stays exactly that string; no secret is ever resolved, so nothing new can reach a report or an AI prompt). The MCP write tools still refuse to rewrite those files, but they now say so: their "not found" names the files skipped for carrying HA tags, instead of implying the script does not exist.
- **`ha_import_blueprint` wrote the downloaded document wherever it asked to be written** — the `domain:` value read out of the *fetched* YAML went straight into the output path, so `domain: ../../` — or an absolute path, which `os.path.join()` lets win outright — dropped remote content outside `/config/blueprints/`. It was the one blueprint tool this release's path-traversal hardening did not cover. The domain is now limited to `automation` / `script` / `template` and the resolved path must sit inside `/config/blueprints/`. The fetch is hardened at the same time: `http(s)` only, every address the host resolves to must be public (loopback, LAN, link-local and metadata addresses refused), redirects re-checked at each hop, 2 MB ceiling — as it stood, an LLM-driven tool could GET anything the Home Assistant host can reach and leave the answer on disk.
- **The panel rendered white cards and a white background in dark mode** — the panel runs in an iframe, which carries the Home Assistant frontend's light base `:root { … }`. The theme was propagated by copying the parent's `html` / `:root` rules plus its inline style wrapped as `html { … }`, a selector that loses the specificity contest against that base rule — and which misses whatever the parent delivers through `document.adoptedStyleSheets`, `--card-background-color` among them. Dark-theme text on white cards, unreadable. Each theme variable's *computed* value is now read off the parent `<html>` and written inline with `!important`, which no stylesheet can override whichever route Home Assistant used to deliver it; `color-scheme` follows too, so scrollbars and native controls match. The sync also re-runs whenever the theme changes instead of only on first load — switching Home Assistant between light and dark now re-themes the panel without leaving the tab.
- **Every refactoring service failed on a split config** — the refactoring module still opened `<config>/automations.yaml` and `scripts.yaml`, which do not exist when those domains point at folders, so the device-id, mode, template, zombie-entity and description fixes all returned `[Errno 2] No such file or directory`. Each now locates the file that holds the target and writes back only there; a backup is named after the file it copies, and `restore_backup` puts it back where it came from instead of writing a config-root file Home Assistant does not read.
- **`packages:` was audited as `<config>/packages/`, whatever `configuration.yaml` said** — the folder was hardcoded, so packages declared elsewhere were never audited while an undeclared `packages/` folder was audited anyway, a `.yml` there was invisible, and a package carrying `!secret` was dropped with no log line at any level. Package files now go through the shared resolver, in the automation audit and in the template-sensor analysis — which also stopped returning nothing on any `configuration.yaml` that contains an `!include`.
- **"YAML file never loaded" could tell you to break your config** — the check reported every `.yml` in a merged folder, including one an explicit `!include` pulls in, which Home Assistant loads fine. Renaming it as advised breaks that include, and the renamed file is then swept up by the `*.yaml` glob with the wrong shape. Files named by an `!include` are no longer reported, and the message no longer prescribes a rename blindly.
- **`ha_update_config_file` could never write a package file** — its allow-list exemption tested `os.path.basename(filename).startswith("packages/")`, and a basename never contains a separator, so the branch was dead and every `packages/` write was refused. It now runs on the resolved path, after the traversal guard.
- **`ha_remove_automation` did not accept a full `automation.<slug>`** — an automation's entity slug comes from its alias and is written nowhere in the YAML, so only a bare `id` resolved, and the alias pass compared against the still-prefixed string. The entity registry now supplies the automation's `id` (which Home Assistant stores as the entity's `unique_id`) for the `entity_id` form.
- **The panel kept stale colours when a theme was edited in place** — the change probe watched two background variables only, so editing `--primary-color` or `--primary-text-color` in the active theme showed nothing until a tab reload. An identity check on the theme definition catches it, still O(1) per state update.
- **`Foo.YAML` in a merged folder** — the extension test now follows the filesystem's case rules, like Home Assistant's own glob: the file is audited on macOS and Windows, where HA loads it, and reported as never loaded on Linux, where HA skips it.
- **Restoring a backup could destroy it** — backup names carry a one-second timestamp, so a second operation within the same second overwrote the first; since `restore_backup` snapshots the current state before restoring, that snapshot could land on the very backup being restored, which then copied onto itself and changed nothing. Colliding names now get a sequence suffix.
- **The MCP "not found" answers re-read the whole domain to build themselves** — after a scan came up empty, `ha_get_script`, `ha_get_scene`, the update and remove tools each walked and re-parsed every file again to work out which ones they had skipped, and again to list the available entries: three full passes over a split folder, on the path where something had already gone wrong. The scan now returns what it searched and what it could not parse, so the error is assembled from the pass that already ran.

### Changed

- **The automation optimizer works on the file that holds the automation** — it read and rewrote `<config>/automations.yaml` unconditionally, so on a split config it found nothing to optimise and its backup snapshotted the wrong file. Backups are now named after the file they copy.
...
- **The startup scan ran on a fixed guess instead of a check** — `startup_delay_seconds` was a plain `asyncio.sleep()` after `EVENT_HOMEASSISTANT_STARTED`, which is wrong in both directions. That event only means every integration's `async_setup_entry()` has returned; slow integrations (Zigbee, Z-Wave, cloud-polling platforms) keep restoring entities and flipping them out of `unavailable`/`unknown` well past it — measured on a real instance, `/api/states` was still growing (401 → 677 → 748 entities) some 35 s after Home Assistant was already answering HTTP 200, so "HA responds" is not a signal that HA is loaded. Too short a delay and the first scan reports still-loading entities as `unavailable_entity`, dropping the health score right after every restart until someone waits it out and rescans by hand to confirm nothing is actually broken; too long and an instance that settled at 20 s still waits the full minute before giving real numbers. The wait is now adaptive: HACA polls the state machine every 2 s and starts the scan once the total entity count **and** the exact set of `unavailable`/`unknown` entity_ids have stopped changing for 10 s, never before 15 s have elapsed. It tracks the set rather than a count on purpose — two entities trading places (one recovering as another drops) leaves a count unchanged while the instance is still very much churning — and the 15 s floor is there because a boot does not register entities continuously: a lull between two integrations looks exactly like a settled instance, and without the floor a short pause right after `EVENT_HOMEASSISTANT_STARTED` would be enough to start the scan before Zigbee/Z-Wave had begun restoring theirs. `startup_delay_seconds` keeps its name, its default (60 s) and its 0–300 range, but becomes the **ceiling** of that wait instead of a fixed sleep: a settled instance is scanned in about sixteen seconds, and one that never settles — a genuinely broken entity can't be waited out — is scanned when the ceiling hits, exactly as before. Set the ceiling below 15 s and it wins, so the option can still force an early scan. The field's label was reworded accordingly in the 13 languages. One limit worth stating: this is a much better signal than a fixed delay, not a proof — a boot that goes completely silent for 10 s and then resumes will still be called settled. Reported by a user, with a diff.
- **The first scan after a restart raised alerts against an empty baseline** — the post-scan notification listener and the Repairs sync both compare the current scan to the previous one, and `_prev_high_issue_keys` starts empty at every setup. The first scan of a session therefore had nothing to compare against and treated *every* issue it found as brand new: a persistent notification listing problems that had been there for weeks, plus a full re-population of the Repairs panel — and, whenever the startup wait above hits its ceiling before Home Assistant has finished restoring entities, a batch of `unavailable_entity` entries for devices that were merely still booting. The first scan of a session is now a **baseline**: it records what it finds and says nothing — no notification, Repairs left exactly as it was — and alerting starts at the second scan, on what has actually changed since. The HACA panel is unaffected: it shows the first scan's results immediately, only the push side waits. A first refresh that fails does not consume the baseline, otherwise the next scan would alert on everything. Raised by the same user alongside the startup-timing report.

- **Deleting a weekly agent report did nothing at all** — `delete_report_session()` looked for files with `glob("report_*")` and a `report_<session_id>` prefix regex. Agent reports are named `agent_report_<timestamp>_score<NN>.md` and are listed under the session id `agent_<timestamp>`, so they matched neither the glob nor the regex: the service deleted nothing, returned `success: false`, and the panel reported it only through a persistent notification — from the Reports tab it looked like the click had simply been ignored. Deletion now derives the session id from the filename with the same rule the listing uses (`report_session_id()`), so what the tab lists is exactly what a delete removes. A truncated id (`20260820` for `20260820_090000`) used to match several sessions by prefix; it now matches none.
- **Deleting a report gave no visible feedback** — you confirmed the dialog and, for the several hundred milliseconds it takes to round-trip and unlink the files, nothing happened on screen; success and failure were announced only through a Home Assistant persistent notification, which is not where you are looking when you are inside the panel. The delete button now shows a spinner and its row dims while the deletion runs, a toast says it is running, and a second toast reports the outcome — `N file(s) deleted`, or the actual error message. On failure the row comes back instead of staying greyed out. One new translation key (`reports.deleting`), hand-translated in the 13 languages.
...
- **`config_auditor.get_report_content` could read files outside the reports directory** — the service joined the caller's `filename` straight onto `haca_reports/`, so a name like `../../<something>.json` escaped it, and the service was registered without an admin requirement, leaving it callable by any logged-in user. It is now admin-only, and both it and the new HTTP view share `resolve_report_path()`: a bare `[A-Za-z0-9._-]` filename, a `.md`/`.json`/`.pdf`/`.html` extension, and a resolved path that is a regular file sitting directly inside `haca_reports/` — anything else is refused. `config_auditor.delete_report` was reachable by any logged-in user for the same reason and now carries the same admin check.
...
- **PDF report generation blocked the event loop** — `_generate_pdf_with_timestamp()` awaited only the final `pdf.output()`; everything before it ran on Home Assistant's event loop: the font search, `add_font()` opening each `.ttf` through fontTools, and the whole page layout. HA's watchdog flagged it on every report (`Detected blocking call to open with args (…/fonts/DejaVuSans.ttf, 'rb') … at report_generator.py, line 479`), and on an SD-card install the loop stalls for as long as the build takes. The build now runs whole in a single executor job — the Markdown and JSON writers already did.
- **Every report was stamped `v1.3.0`** — the version printed in the Markdown and PDF footers was a hardcoded literal, so a 1.7.6 install still produced reports announcing v1.3.0. It now comes from `manifest.json` through `async_get_integration()`, resolved once per generator and cached.
