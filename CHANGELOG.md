...
- **A fix applied from the panel could disable the automation it fixed, and report success** — removing a zombie entity could leave a state trigger with no entity, and converting a device condition such as *door open* to a state condition dropped the state it expects; Home Assistant disables such an automation at the next reload, without an error. The panel's fixes now go through the same check as the MCP tools: one Home Assistant would refuse is not written, and the panel shows its reason. A failed reload after a zombie-entity fix also puts the file back.
