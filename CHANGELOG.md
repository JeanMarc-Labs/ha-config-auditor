...
- **The entity reference walk froze Home Assistant for ten seconds at a time** — the step that reads every automation and script to work out which entities they use ran entirely on the event loop: 9.9 s of a 10.0 s entity analysis on a Raspberry Pi 3, in one freeze during which Home Assistant answered nothing. It runs in a worker thread now, and the second pass that re-read each configuration as text — the net that catches entity ids hiding under unexpected keys — is folded into the same walk, so the configuration is read once instead of twice: 38% less work for that step. **The scan does not get much shorter; it stops blocking.**
