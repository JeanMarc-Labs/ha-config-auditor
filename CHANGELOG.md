...
- **Duplicate detection compared every automation to every other one** — the near-duplicate check ran a Jaccard similarity over all n(n−1)/2 pairs, in a synchronous method that cannot hand the event loop back while it runs, and the new scan instrumentation measured that tail of the automation analysis inside a 12.6 s freeze. Identical automations are now compared once instead of pair by pair, and each one only meets those sharing one of its rarest structural tokens: same findings, same order, 2 to 11 times faster depending on how many automations there are.
