...
- **The panel's zombie-entity fix could disable the automation it fixed, and report success** — removing the entity could leave a state trigger with no entity, which Home Assistant disables on a reload that still succeeds; and a failed reload left the edited file in place. The fix now goes through the same check as the MCP tools: one Home Assistant would refuse is not written and the panel shows its reason, and a failed reload puts the file back.
